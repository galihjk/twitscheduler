...
user_id='$userid' 
and input_time > now() - interval $txt_quota_seconds second - interval $media_quota_seconds second    
order by input_time asc";
$my_posts = f("db.q")($q);

$base_quota_max = f("get_config")("base_quota_max",1);
$base_quota_seconds = f("get_config")("base_quota_seconds",1);
$q = "select input_time from posts where input_time > now() - interval $base_quota_seconds second order by input_time asc";
...
if(empty($data_base_quota)){
    ...
    $base_tunggu_kuota = 5+($base_quota_seconds - (time()-strtotime($data_base_quota[0]['input_time'])));
}
